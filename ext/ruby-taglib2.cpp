...
StringAttribute(albumArtist, AlbumArtist)
StringAttribute(album, Album)
StringAttribute(comment, Comment)
StringAttribute(genre, Genre)
StringAttribute(grouping, Grouping)
StringAttribute(composer, Composer)
UIntAttribute(year, Year)
UIntAttribute(track, Track)
UIntAttribute(cdNr, CDNr)
UIntAttribute(totalCDs, TotalCDs)
UIntAttribute(totalTracks,TotalTracks)
UIntAttribute(bpm, BPM)
...
extern "C" void Init_taglib2(void)
{
	...
	AttributeMethods(albumArtist, AlbumArtist)
	AttributeMethods(album_artist, AlbumArtist)
	AttributeMethods(grouping, Grouping)
	AttributeMethods(composer, Composer)
	AttributeMethods(bpm, BPM)
	...
	AttributeMethods(cdNr, CDNr)
	AttributeMethods(cd_nr, CDNr)
	AttributeMethods(cd_number, CDNr)
	AttributeMethods(totalCDs, TotalCDs)
	AttributeMethods(total_cds, TotalCDs)
	AttributeMethods(totalTracks, TotalTracks)
	AttributeMethods(total_tracks, TotalTracks)
